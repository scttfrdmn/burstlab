#!/bin/bash
# =============================================================================
# efs-slurmctld-prolog.sh — PrologSlurmctld for ephemeral EFS
#
...
# Wait for EFS to be in 'available' state before adding mount target.
# IncorrectFileSystemLifeCycleState is returned if this is skipped.
...
# home dirs (chmod 700).
STATE_DIR="/opt/slurm/var/efs-state/${SLURM_JOB_USER}"
mkdir -p "${STATE_DIR}"
chmod 755 "${STATE_DIR}"
...
EOF
# chown requires root on NFS — use world-writable so the job user can write
