# =============================================================================
# BurstLab Gen 4 — Packer AMI Template
# Ubuntu 22.04 LTS + Slurm 23.11.x + AWS deps
#
# Key differences from Gen 2 (Rocky 9 + 23.11):
#   - Ubuntu 22.04 LTS (Jammy Jellyfish): uses 'apt' package manager (not dnf),
#     package names use -dev suffix (not -devel), AppArmor (not SELinux),
#     ufw firewall (not firewalld), systemd units in /etc/systemd/system
#   - Slurm 23.11: same version as Gen 2, adds SlurmctldParameters=idle_on_node_suspend,
#     slurmrestd daemon, partition-level ResumeTimeout/SuspendTime support
#   - Python: system python3.10 has pip; boto3 installed directly
#   - PAM modules: /usr/lib/x86_64-linux-gnu/security (not /usr/lib64/security)
#   - FSx Lustre: BLOCKED (AWS repo has no Ubuntu packages, like Gen 3/Rocky 10)
#   - EFS: fully functional (standard NFS4)
#
# Build: packer build ubuntu2204-slurm2311.pkr.hcl
# =============================================================================
...
# -----------------------------------------------------------------------------
# Source: Amazon EBS-backed Ubuntu 22.04 LTS instance
# -----------------------------------------------------------------------------
...
source "amazon-ebs" "ubuntu2204" {
  ...
  # Ubuntu 22.04 LTS official AMI publisher: 099720109477 (Canonical)
  # Naming convention: ubuntu/images/hvm-ssd/ubuntu-jammy-22.04-amd64-server-*
  source_ami_filter {
    filters = {
  ...
  ami_name        = "burstlab-gen4-ubuntu2204-slurm${var.slurm_version}-{{timestamp}}"
  ami_description = "BurstLab Gen 4: Ubuntu 22.04 LTS + Slurm ${var.slurm_version} from source. All node roles."

  tags = {
    Name         = "burstlab-gen4-ubuntu2204-slurm${var.slurm_version}"
    Project      = "burstlab"
    Generation   = "gen4"
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      #
      # Ubuntu 22.04 uses apt/apt-get. Enable universe repo for build tools.
      # =======================================================================
      ...
      #   iptables, iptables-services, nftables
      ...
      # iptables: Ubuntu 22.04 uses nftables by default but iptables available
      # for compatibility with NAT init script. iptables-persistent for rule saving.
      "sudo DEBIAN_FRONTEND=noninteractive apt-get install -y iptables iptables-persistent nftables",

      # Python 3.10 (Ubuntu 22.04 default) with pip
      ...
      # cgroup_v2.so is not produced. Ubuntu 22.04 uses cgroup v2 exclusively,
      ...
      # Same IDs as Gen 1-3 to ensure EFS ownership is consistent if a site
      ...
      #
      # Same prefix strategy as Gen 1-3:
      ...
      #
      # Same units as Gen 2. All point SLURM_CONF to the EFS path.
      ...
      #
      # Ubuntu 22.04 ships Python 3.10 as the default python3. boto3 is installed
      # to the system Python directly.
      #
      # PEP 668 note: Ubuntu 22.04 enforces "externally managed environment"
