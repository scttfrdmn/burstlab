...
```ini
NodeName=aws-burst-[0-9] CPUs=8 RealMemory=31500 Weight=1 State=CLOUD
...
**Node naming convention:** `aws-burst-[0-9]` follows Plugin v2's naming scheme: `{PartitionName}-{NodeGroupName}-{index}`. With `PartitionName=aws` and `NodeGroupName=burst` in `partitions.json`, the names are `aws-burst-0` through `aws-burst-N`. This naming is not optional — the names must match exactly between `slurm.conf` (node definitions), `partitions.json` (plugin config), and the EC2 `Name` tag set by `resume.py`.
...
```ini
PartitionName=all Nodes=compute[01-04],aws-burst-[0-9] Default=NO MaxTime=INFINITE State=UP
```
