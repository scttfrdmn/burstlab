...
**AccountingStorageEnforce=associations,limits** means jobs submitted by users who are not registered in the accounting database will be rejected. For a lab cluster, this requires that you have at minimum a root-level account and the `centos` user associated with it. The head node cloud-init script runs:

```bash
sacctmgr -i add cluster burstlab-gen1
sacctmgr -i add account burstlab Description="BurstLab" Organization="BurstLab"
...
This directive controls what `sinfo` shows to non-privileged users. Without `PrivateData=CLOUD`, cloud nodes in the `CLOUD` (powered off) state are hidden from `sinfo` output by default. Users and SAs would see an empty cloud partition and have no visibility into the burst node inventory.
...
**ResumeProgram** is called by slurmctld when a job is submitted to the cloud partition and there are no available nodes. Slurm passes the node name(s) as an argument. `resume.py` translates the node names into an EC2 `CreateFleet` request:

1. Parses the node names (e.g., `cloud-burst-[0-2]`)
...
4. Sets the `Name` tag on new instances to the Slurm node name (e.g., `cloud-burst-0`)
...
**What to set this to:** On `m7a.xlarge`, the sequence from `CreateFleet` call to `slurmd` registration is:
...
$ sinfo -p cloud
PARTITION AVAIL  TIMELIMIT  NODES  STATE NODELIST
cloud        up    4:00:00      1  down*  cloud-burst-0

$ scontrol show node cloud-burst-0 | grep Reason
...
```ini
NodeName=compute[01-04] CPUs=2 RealMemory=7500 State=IDLE
...
**CPUs=2** matches the vCPU count of the `m7a.large` instance type used for compute nodes (2 vCPUs). Setting this correctly is important for job packing — if you declare `CPUs=4` on a 2-vCPU node, Slurm will allocate up to 4 CPUs per job on that node, causing jobs to compete for the same physical cores.

**RealMemory=7500** is the usable memory in MB. `m7a.large` has 8 GB = 8192 MB. We deduct ~700 MB for the OS, munge, slurmd, and system overhead. Setting `RealMemory` too high causes jobs to request more memory than the node can actually provide, leading to OOM kills that are difficult to diagnose.
...
**MaxTime=4:00:00** caps job runtime on burst nodes at 4 hours. This is a cost control measure — a job that runs indefinitely on an `m7a.xlarge` would accumulate unexpected charges. 4 hours is long enough for most demo and research computing jobs, short enough to prevent runaway costs.

**PartitionName=cloud ... Default=NO** makes the cloud partition non-default. Users must explicitly request `--partition=cloud` to burst. This matches typical university policy: bursting is an opt-in resource, not the default allocation.

**Node naming convention:** `cloud-burst-[0-9]` follows Plugin v2's naming scheme: `{PartitionName}-{NodeGroupName}-{index}`. With `PartitionName=cloud` and `NodeGroupName=burst` in `partitions.json`, the names are `cloud-burst-0` through `cloud-burst-N`. This naming is not optional — the names must match exactly between `slurm.conf` (node definitions), `partitions.json` (plugin config), and the EC2 `Name` tag set by `resume.py`.
...
```ini
PartitionName=all Nodes=compute[01-04],cloud-burst-[0-9] Default=NO MaxTime=INFINITE State=UP
```
...
For a demo, having both `local`, `cloud`, and `all` partitions lets an SA show different scenarios:
- `--partition=local`: always on-prem, guaranteed immediate execution (up to capacity)
- `--partition=cloud`: always a burst node (for showing the cloud transition)
