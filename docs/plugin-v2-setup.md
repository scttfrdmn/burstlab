...
```
ERROR: SlurmConf.ResumeTimeout in config.json (600) does not match
       slurm.conf value (650). These must match exactly.
```
...
**Examples:**
- `PartitionName=aws`, `NodeGroupName=burst` → nodes named `aws-burst-0`, `aws-burst-1`, ... (BurstLab default)
...
|---|---|---|---|
| `CPUs` | `"8"` | m7a.2xlarge | 8 vCPUs |
| `RealMemory` | `"31500"` | m7a.2xlarge | 32,768 MB - ~1,268 MB OS overhead |
...
NodeName=aws-burst-[0-9] CPUs=8 RealMemory=31500 Weight=1 State=CLOUD Feature=aws
PartitionName=aws Nodes=aws-burst-[0-9] Default=NO MaxTime=4:00:00 State=UP
```
...
1. `resume.py` is called with argument `aws-burst-0`
2. `resume.py` calls `ec2:CreateFleet` with the launch template
3. `resume.py` calls `ec2:CreateTags` on the new instance: `Name=aws-burst-0`
...
5. Cloud-init reads the `Name` tag from IMDS:
   ...
   # SLURM_NODENAME = "aws-burst-0"
   ```
6. Cloud-init sets the hostname to `aws-burst-0`
...
aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=aws-burst-0" \
...
```bash
grep -i "aws-burst-0" /var/log/slurm/slurmctld.log | tail -20
```
...
```bash
scontrol show node aws-burst-0 | grep -E "State|Reason"
```
...
```
slurmctld: error: validate node aws-burst-0: Invalid configuration
```
