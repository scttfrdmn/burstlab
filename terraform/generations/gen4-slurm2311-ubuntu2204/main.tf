# =============================================================================
# ROOT MODULE — BurstLab Gen 4
# Generation: gen4-slurm2311-ubuntu2204
# OS: Ubuntu 22.04 LTS
# Slurm: 23.11.x
# =============================================================================
# Same six modules as Gen 1-3 (VPC, IAM, shared-storage, head-node, burst-config,
# compute-nodes). The generation-specific differences are:
#   - AMI: packer build ami/ubuntu2204-slurm2311.pkr.hcl
#   - Config templates: configs/gen4-slurm2311-ubuntu2204/
#   - slurm.conf: SlurmctldParameters=idle_on_node_suspend, TaskPlugin=task/cgroup
#   - cgroup.conf: CgroupPlugin=cgroup/v2 (Ubuntu 22.04 uses cgroup v2 exclusively)
...
#   ssh -i ~/.ssh/<key>.pem ubuntu@$(terraform output -raw head_node_public_ip)
# =============================================================================
...
resource "null_resource" "burst_node_cleanup" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = <<-EOT
      echo "=== BurstLab Gen 2: terminating burst nodes before destroy ==="
