# =============================================================================
# ROOT MODULE VARIABLES — BurstLab Gen 4
# Ubuntu 22.04 LTS + Slurm 23.11.x
# =============================================================================
...
variable "cluster_name" {
  description = "Name prefix applied to every resource. Use 'burstlab-gen4' to distinguish from other generations in the same account."
  type        = string
  default     = "burstlab-gen4"
}
...
variable "head_node_ami" {
  description = "AMI ID for the head node. Must be the output of: packer build ami/ubuntu2204-slurm2311.pkr.hcl. AMI ID changes on each Packer build — get the latest with: aws ec2 describe-images --owners self --filters 'Name=name,Values=burstlab-gen4-*'"
  type        = string
