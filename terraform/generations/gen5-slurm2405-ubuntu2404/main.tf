...
resource "null_resource" "burst_node_cleanup" {
  ...
  provisioner "local-exec" {
    when    = destroy
    command = <<-EOT
      echo "=== BurstLab Gen 5: terminating burst nodes before destroy ==="
